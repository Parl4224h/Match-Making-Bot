...
export const processMMR = async (users: GameUser[], scores: number[], scoreLimit: number): Promise<number[][]> => {
    let teamA: StatsInt[] = [];
    let teamB: StatsInt[] = [];
    for (let user of users) {
        if (user.team == 0) {
            teamA.push(await cacheController.getStatsByUserId(user.dbID) as StatsInt);
        } else {
            teamB.push(await cacheController.getStatsByUserId(user.dbID) as StatsInt);
        }
    }
    ...
    for (let i = 0; i < teamAChanges.length; i++) {
        ...
        await cacheController.updateStats(teamA[i]);
        await cacheController.updateStats(teamB[i]);
    }

    // Update the ranks
    let i = 1;
    for (let stat of await cacheController.getStats()) {
        if (stat.gamesPlayed >= 10) {
            stat.rank = i;
            i++;
            await cacheController.updateStats(stat);
        }
    }
